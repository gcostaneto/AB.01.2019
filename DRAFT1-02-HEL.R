...
#'----------------------------------------------------------------------
GY.df <- readRDS("GYhel452.rds")
levels(GY.df$env) <- c("NM","SO","PM","IP","SE")
...
LA <- t(chol(read.table("GENOTYPE-GBkernel.txt") + diag(5E-5,nrow = 452)))

# Environmental markers                                                                                         
#----------------------------------------------------------------------
W <- data.frame(as.matrix(readRDS("WHel.full")))
...
#----------------------------------------------------------------------
Op <- t(chol(diag(1,nrow=5)));#rownames(Op) = colnames(Op) = colnames(O)
